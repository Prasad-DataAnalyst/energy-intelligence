...
def write_env(channel_id: str):
    """MERGE the YouTube settings into .env — never rewrite the whole file.

    The old version replaced .env with a template, silently wiping
    ANTHROPIC_API_KEY, GMAIL_APP_PASSWORD, and every other key the user had
    added (this actually happened in production and killed the pipeline)."""
    banner("Updating .env configuration (merge — existing keys preserved)…")

    updates = {
        "YOUTUBE_CLIENT_SECRETS_FILE": "client_secrets.json",
        "YOUTUBE_CHANNEL_ID":          channel_id,
    }
    defaults = {                       # only added if not already present
    ...
    for line in lines:
        ...
        if key in updates:
            ...
            if key:
                seen.add(key)
    for key, val in {**updates, **defaults}.items():
        if key not in seen:
            out.append(f"{key}={val}")

    ENV.write_text("\n".join(out) + "\n")
    print(f"  ✓  .env updated (existing keys like ANTHROPIC_API_KEY preserved)")
